...
public class Day04_5 {//c s
	public static void main(String[] args) {//m s
		...
		//데이터 호출
			//배열명 : 배열 메모리 주소값 
			//1. 배열명[인덱스번호] : 해당 인덱스번호의 데이터호출
		//데이터 변경[대입]
			//1. 배열명[인덱스번호]=새로운 데이터
		//배열의  길이(갯수)
			//1. 배열명.length : 배열내 길이[변수의 갯수]
		//for과 배열 활용
			//for(자료형 임시변수 : 배열명){ }
			//배열내  0번 인덱스부터 끝까지 하나씩 임시변수에 대입 반복처리
		...
		//* 83, 90, 87 이라는 int형 데이터 3개를 저장하는 배열 선언
			...
			//87-> 3번째  인덱스에 저장
		//배열 출력
		System.out.println("scores : "+scores);//배열이름호출-> 메모리주소값
		...
		int[]정수배열1=new int[3];
			...
			//int[3] : 자료형[길이] : 저장개수
		//int형 변수  3개를 저장 할 수 있는 배열 선언[메모리 할당]
		for(int i=0;i<3;i++) {
		...
		for(int i=0;i<정수배열1.length;i++) {
			//i는 0부터정수배열1의 길이 미만까지 1씩 증가하면서  반복처리
			System.out.println(정수배열1[i]);
		}
		
		//for문 변경 활용
			//배열자료형 변수명 : 배열명--> 하나씩  반복하면서담아둠.
		for(int temp:정수배열1) {
			// 임시변수 ; 배열명
			// 배열 내 인덱스 0번부터 끝까지 하나씩 데이터 temp  저장반복
				//1.  정수배열1[0]-> temp 저장
				//2.  정수배열1[1]-> temp 저장
				//3.  정수배열1[2]-> temp 저장
